            ...
            if wants_aero(seg):
                ...
                seg_f.setdefault("pressure_scale",
                                 [[0.0, 0.12], [0.45, 1.0], [0.55, 1.0],
                                  [1.0, 0.15]])
                # 협착 면적도 **대칭**이어야 한다. constriction_area 의 기본값은
                # "좁게 유지하다가 60 ms 만에 모음 면적으로 확 연다" 인데, 그건
                # 자음이 모음으로 넘어가는 **해제** 동작이다. 뒤에 모음이 없는
                # 독립 마찰음에는 해제가 없다 - 혀가 다가갔다가 물러날 뿐이다.
                # 그대로 두면 압력 곡선을 아무리 만져도 같은 자리에서 끊긴다:
                # 면적이 0.1 -> 3.0 으로 30 배 열리면 유속이 30 배 떨어지고
                # 진폭 ∝ U^1.25 라 30 ms 만에 -44 dB 로 절벽이 된다(측정).
                seg_f.setdefault("constriction_area",
                                 [[0.0, 0.45], [0.42, 0.10], [0.58, 0.10],
                                  [1.0, 0.45]])
