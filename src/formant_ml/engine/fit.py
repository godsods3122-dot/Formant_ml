...
#
# **6.0 의 출처 — 코퍼스 구간 셋으로 확정했다** (docs/MEASUREMENTS.md §26).
# 자는 첨두 밀도의 목표 대비 비(1.00 이 같음)와 검증 시드의 시간평균 스펙트럼이다.
#
#   sigma      s34    ss00   sv11   |  검증 스펙 평균   학습−검증 격차
#   0 (예전)   3.53x  7.43x 11.14x  |      40.0            14.7 %p
#   3 ms       1.91   3.27   3.12   |      42.5             8.1
#   4.5 ms     1.05   1.63   2.18   |      42.9             4.3
#   **6 ms**   1.05   1.07   1.37   |      42.3             4.6
#
# 6 ms 만이 **세 구간 모두**에서 첨두를 목표 근처로 데려온다. 검증 스펙트럼은
# 4.5 와 통계적으로 같고(구간별 표준편차 1.2~3.0), 셋 다 sigma 0(40.0)을 이긴다.
# 저역도 같이 정리된다 — 80~150 Hz 초과가 +13.1/+16.4/+4.5 에서 +1.5/−5.5/−8.1 로.
#
# **그런데 이 값을 기본으로 켰다가 되돌렸다 (docs/MEASUREMENTS.md §27).**
# 청취: "전혀 치찰음이라곤 생각도 안 나는 수준. 그냥 쓰레기 기계음." 위 자들이
# 전부 좋아지는 동안 **치찰음의 정체성**(앞공동 정점·무게중심)이 무너졌는데, 그것을
# 재는 자를 판정에 안 넣었다. §14 가 "대역 에너지표가 맞아도 무게중심이 1 kHz
# 어긋나면 /s/ 가 /ʃ/ 로 들린다" 고 이미 적어 둔 것을 놓쳤다.
#
# **기본값은 0 이다.** 켜려면 무게중심·치찰도를 합격 조건에 넣고 다시 정하라.
W_SMOOTH_MS = 0.0
