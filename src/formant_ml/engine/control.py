...
_P: list[ParamSpec] = [
    ...
    # 상한 0.05 -> 0.010. **생리 범위 밖으로 나가지 못하게 한다.** 모달 발성의 지터는
    # 0.2~1 % 이고 그 위는 병적 음성의 값이다. 상한이 5 % 이던 동안 적합기가 1.5~1.6 %
    # 를 골랐는데(실측 yang_00000101/40), 그건 소스가 못 맞추는 비주기성을 주기 요동으로
    # 떠넘긴 것이지 이 화자의 목소리가 그렇다는 뜻이 아니다. 이 값은 학습 데이터로
    # 그대로 나가므로 물리 범위를 지켜야 한다.
    ParamSpec("jitter", "ratio", 0.0, 0.010, 0.004, False, "주기 요동 (모달 0.2~1 %)"),
