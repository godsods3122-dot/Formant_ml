...
class FilterConfig:
    ...
    # **20 kHz 는 중립이 아니다 — 재 봤다.** r=exp(-pi*bw/fs)=0.073 이라
    # DC 정규화 응답이 나이퀴스트에서 **+2.41 dB** 다(예전 주석의 "±0.6 dB" 는
    # 틀렸다). 렌더로 확인하면 빈 슬롯 2 개가 4~7 k +1.3 / 7~9.5 k +2.9 /
    # 9.5~12 k +3.7 dB 를 들어 올린다. 즉 '극 없음' 이 실제로는 **상승 셸프**다.
    #
    # 진짜 중립은 r=0, 즉 bw -> inf 다. 40 kHz 면 0.18 dB, 80 kHz 면 0.00 dB.
    # 값을 안 바꾸고 두는 이유: 이 상수 위에서 `tilt`·기식 세기가 골라져 있어서
    # 바꾸면 소리가 같이 움직인다. **바꿀 때는 그 둘을 함께 다시 골라라.**
...
class NoiseConfig:
    ...
    #
    # **기본값이 0 인 이유를 먼저 읽어라 — 아래 논증과 값이 어긋나 보인다.**
    # 물리적으로는 켜는 게 맞는데(아래), 켜 봤더니 마찰음이 실측을 밑돌았다:
    # 대역 게인(`noise_bands`)이 이미 방사를 흡수한 채 적합돼 있어서 이중계산이
    # 된다. 그래서 설정으로만 빼 두고 기본은 0 이다. 켜려면 대역 게인을
    # **방사 없이 다시 적합**해야 한다. (커밋 6133a67)
    #
