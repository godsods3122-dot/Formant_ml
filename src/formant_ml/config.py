...
class FilterConfig:
    ...
    # 이 번호(0-based) 이상의 포먼트는 프레임마다 움직이지 않고 화자 상수로 둔다.
    # 5 kHz 위의 스펙트럼은 혀 위치가 아니라 고정 해부(이상와, 후두강, 치열,
    # 입술 방사)가 지배한다. 1D 평면파 근사도 그 위에서는 어차피 성립하지 않는다.
    # 움직이게 두면 모델이 잡음을 그리로 흘려보내고, 학습만 어려워진다.
    formant_fixed_from: int = 5
