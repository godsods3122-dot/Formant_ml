...
#: 독립 마찰음에서 가청 /s/ 는 세그먼트의 약 53 % 다 — 나머지는 압력이 임계
#: 유속에 못 미쳐 소리가 안 나는 앞뒤 램프다(그게 페이드 인/아웃의 실체다).
#: 그래서 1.1 s 짜리 /s/ 를 들으려면 세그먼트를 2.1 s 로 잡는다.
#:
#: 48 % 에서 올렸다. 난류 소스 기울기가 이제 입자속도를 따라가면서
#: (`aeroacoustic.source_tilt_shift`) 압력이 낮은 앞뒤 램프에서도 고역이 조금
#: 남아, 고역 포락선이 0.1 을 넘는 구간이 그만큼 길어졌다. 물리가 바뀐 게
#: 아니라 **같은 소리의 가청 구간이 길어진** 것이라 여기서 되받는다
#: (재측정: 2.08 s 세그먼트 -> 가청 1109 ms, 실측 1097 ms).
AUDIBLE_FRAC = 0.53
