...
* **메모리 걸림쇠는 "지금 여유" 가 아니라 "건당 정점" 으로 걸어라.** 여섯 건을 여유
  8 GB 를 확인하며 하나씩 띄웠는데, 넷이 다 뜬 뒤 **함께 자라서** 여유가 4 GB 까지
  떨어졌고 시스템이 큐 셸을 죽였다. 그런데 **적합 프로세스는 안 죽고 고아로 계속
  돌았다** — 결과는 나오지만 알림이 안 온다. 반드시 `tasklist` 로 확인하라.
* **코드가 바뀌면 건당 RSS 를 다시 재라.** 복원 구조 + 다중 해상도 + `--balance` 판은
  건당 **2.1~4.5 GB** 다 (그 전 판은 1.5~2.4 GB). 32 GB 기계에서 여유 8 GB 를 남기려면
  **동시 4 건**이 상한이다.
* **0.3.19 판(성도 궤적 평활 + 무성 벌점 + reseed)은 건당 3.3~6.1 GB** 다 (위상 단계에서 가장 크다).
  네 건을 띄웠더니 가용 3 GB·가상 여유(commit) 3.9 GB 까지 떨어져 한 건을 멈췄다. **동시 3 건,
  가용 14 GB 걸림쇠**로 돈다(`out/_tmp/waveT.sh`). 가상 여유가 바닥나면 할당이 실패해 적합이
  흔적 없이 죽는다 — 물리 가용만이 아니라 `FreeVirtualMemory` 도 본다.
